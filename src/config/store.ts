...
export class CheckpointStore {
  ...
  async read(): Promise<CheckpointState> {
    const updatedRow = this.db
      .prepare("SELECT value FROM meta WHERE key = 'updatedSince'")
      .get() as { value: string } | undefined;
    const seenRows = this.db
      .prepare("SELECT id FROM seen_messages ORDER BY seen_at DESC LIMIT ?")
      .all(this.limit) as { id: string }[];
    const failedRows = this.db
      .prepare(
        "SELECT message_id as messageId, room_id as roomId, sender_name as senderName, sent_at as sentAt, failed_at as failedAt, reason FROM failed_messages LIMIT ?",
      )
      .all(this.failureLimit) as FailedMessageRecord[];
    return {
  ...
  async write(state: CheckpointState): Promise<void> {
    ...
    try {
      if (state.updatedSince) {
        this.db
          .prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('updatedSince', ?)")
          .run(state.updatedSince);
      }
